...
fn empty_input_gives_no_blocks() {
    assert_eq!(list_task_blocks(""), Vec::<String>::new());
}
...
[x] task two
";
    let result: String = list_task_blocks(input).into_iter().take(2).collect();
    assert_eq!(result, expected);
}
...
[ ] task two
";
    let result: String = list_task_blocks(input).into_iter().take(99).collect();
    assert_eq!(result, expected);
}
