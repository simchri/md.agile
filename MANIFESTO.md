...
Mdagile is compromise between these approaches. You can quickly and easily take notes - but they will be shared with everyone. You can keep a task completely informal (just a bullet point) - or make it adhere to a predefined workflow. The decision can be made task by task.

## Trust but Control
Centralized task management allows to restrict lifecycle transitions to specific users. This is difficult when everything is text based. Mdagile provides some constraints, but does not aim to be "secure".
