...
").unwrap();
    ...
    assert!(!stdout.contains("README.md"), "non-agile file should be excluded: {stdout:?}");
}
...
").unwrap();
    ...
    assert!(line.starts_with("my.agile.md"), "line should start with filename: {line:?}");
    assert!(line.contains("  "), "should have double-space separator: {line:?}");
    assert!(line.ends_with("my.agile.md"), "line should end with full path: {line:?}");
}
...
").unwrap();
    ...
    assert!(out.status.success());
    let stdout = String::from_utf8(out.stdout).unwrap();
    assert_eq!(stdout.lines().count(), 1, "expected exactly 1 line: {stdout:?}");
}
