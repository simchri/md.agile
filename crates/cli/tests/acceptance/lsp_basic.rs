...
",
    ...
    let init = serde_json::json!({
        ...
        "id": 1,
        "method": "initialize",
        "params": { "processId": 1234, "rootUri": root_uri, "capabilities": {} }
    });
    send_lsp_message(&mut stdin, &init.to_string()).unwrap();
    let _init_response = read_lsp_response(&mut reader).unwrap();
    send_lsp_message(
...
";
    ...
    send_lsp_message(&mut stdin, &did_open.to_string()).unwrap();
    // Discard the publishDiagnostics notification.
    read_notification(&mut reader, "textDocument/publishDiagnostics");

    // Request GoTo Definition with cursor on the `@` of `@alice`.
    ...
    send_lsp_message(&mut stdin, &goto_req.to_string()).unwrap();
...
";
    ...
    // Cursor on 'b' of `@backend` (column 12, after the `@` at 11).
    let goto_req = serde_json::json!({
        "jsonrpc": "2.0",
        "id": 2,
        "method": "textDocument/definition",
