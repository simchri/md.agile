...
- Rank (alt: Priority Rank): A top-level Task's implicit position among its Todo/Done siblings, derived from where it sits in the project's files (see numbered `50_current`/`60_backlog` files). Do not confuse with Order Number (see Ordered Task, above): Rank is never written explicitly in a Task file, whereas an Order Number is an explicit `1.`/`2.` prefix on Subtasks.
- Task Address: The numeric (or dotted, e.g. `1.2`) identifier `agile task next`/`done`/`undone`/`previous` use to name one specific Task or Subtask, printed alongside it in `agile task next`/`previous` output. A top-level Task's own Task Address is always its Rank (its position counting every Todo top-level Task in document order) — this stays the same regardless of `--mine`/`--as` filtering, since `agile task done`/`undone` never take an identity filter and must independently resolve the same address to the same Task. `--mine`/`--as` change which Task `agile task next N` *selects* (the Nth Task eligible for that identity, skipping ineligible ones), but never change the Task Address *printed* for the Task once found. A dotted Task Address's later segments are a Subtask's Order-Number-style position among its direct siblings (unaffected by eligibility, since only top-level Tasks are identity-filtered).
